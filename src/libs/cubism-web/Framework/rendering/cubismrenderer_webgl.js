...
export class CubismClippingManager_WebGL {
    ...
    _currentMaskRenderTexture; // マスク用レンダーテクスチャのアドレス
    _maskRenderTextures; // レンダーテクスチャのリスト
    _maskColorBuffers; // マスク用カラーバッファーのアドレスのリスト
...
export class CubismRendererProfile_WebGL {
    ...
    _lastArrayBufferBinding; ///< モデル描画直前の頂点バッファ
    _lastElementArrayBufferBinding; ///< モデル描画直前のElementバッファ
    _lastProgram; ///< モデル描画直前のシェーダプログラムバッファ
    _lastActiveTexture; ///< モデル描画直前のアクティブなテクスチャ
    _lastTexture0Binding2D; ///< モデル描画直前のテクスチャユニット0
    _lastTexture1Binding2D; ///< モデル描画直前のテクスチャユニット1
    _lastVertexAttribArrayEnabled; ///< モデル描画直前のテクスチャユニット1
    _lastScissorTest; ///< モデル描画直前のGL_VERTEX_ATTRIB_ARRAY_ENABLEDパラメータ
    _lastBlend; ///< モデル描画直前のGL_SCISSOR_TESTパラメータ
    _lastStencilTest; ///< モデル描画直前のGL_STENCIL_TESTパラメータ
    _lastDepthTest; ///< モデル描画直前のGL_DEPTH_TESTパラメータ
    _lastCullFace; ///< モデル描画直前のGL_CULL_FACEパラメータ
    _lastFrontFace; ///< モデル描画直前のGL_CULL_FACEパラメータ
    _lastColorMask; ///< モデル描画直前のGL_COLOR_WRITEMASKパラメータ
    _lastBlending; ///< モデル描画直前のカラーブレンディングパラメータ
    _lastFBO; ///< モデル描画直前のフレームバッファ
    _lastViewport; ///< モデル描画直前のビューポート
    gl;
}
/**
...
export class CubismShaderSet {
    ...
    uniformMultiplyColorLocation; // シェーダープログラムに渡す変数のアドレス（MultiplyColor）
    uniformScreenColorLocation; // シェーダープログラムに渡す変数のアドレス（ScreenColor）
}
